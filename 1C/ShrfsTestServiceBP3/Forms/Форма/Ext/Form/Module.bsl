...
Процедура PostContrНаСервере()
    ...
    Пока Выборка.Следующий() Цикл
		...
		data = data + "<record>";
		data = data + "<code>"+Выборка.Код+"</code>";
	...
	Для Каждого el из jr Цикл
		...
		Если Напр = "Расход" Тогда
			...
			Иначе
				...
				// calc it from input data!
				т.СтавкаНДС = Перечисления.СтавкиНДС.НДС18; //  20
				т.СуммаНДС = СуммаНДС;
...
Процедура LoadDocs(Команда)
	...
	Request.SetBodyFromString(res,"windows-1251");
...
&НаСервере
Процедура GetWaresНаСервере()
	data = "<?xml version=" + """" + "1.0" + """" + " encoding=""windows-1251""?>" 
	+ char(13) + char(10) + "<wares>";
    Запрос = Новый Запрос;
    Запрос.Текст = 
        "ВЫБРАТЬ *
        |ИЗ
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Сообщить(
			Выборка.Код 
			+ " " 
			+ Выборка.Наименование + " (" + Выборка.НоменклатурнаяГруппа + ") " + Выборка.ВидНоменклатуры);
    КонецЦикла;
